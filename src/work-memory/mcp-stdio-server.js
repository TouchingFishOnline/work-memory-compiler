...
function runWorkMemoryMcpServer({ toolHost }) {
  const reader = createMessageReader(process.stdin);
  const resources = buildToolResources(toolHost.listTools());
...
function buildToolResources(toolCatalog) {
  ...
  const resources = [
    {
      uri: "work-memory://tools/index",
      name: "Work Memory Compiler Tool Index",
      description: "Overview of Work Memory Compiler MCP tools with schemas.",
  ...
  for (const tool of tools) {
    resources.push({
      uri: `work-memory://tools/${tool.name}`,
...
function buildToolIndexMarkdown(tools) {
  const lines = [
    "# Work Memory Compiler Tools",
    "",
    "These are Work Memory Compiler MCP tools.",
    "",
  ];
  for (const tool of tools) {
